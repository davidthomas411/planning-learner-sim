...
The 2D nested planner, high-level search oracle, matched dataset pilot, and 64-cubed implicit 3D prototype are executable. Ten tests pass. The latest 3D example reaches all provisional synthetic constraints after four recorded manual beam/priority edits, and the implicit operator has been benchmarked at 64-, 96-, and 128-cubed. The endpoint-versus-trajectory learner comparison and direct four-A100 benchmark have not started. See `CURRENT_RESULTS.md` for the measured values and the boundary between demonstrated mechanics and untested scientific claims.
