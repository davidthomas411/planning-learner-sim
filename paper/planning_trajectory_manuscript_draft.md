...
The prostate development and primary synthetic grid will contain 64 × 64 × 64 voxels. The policy image encoder will receive 32 × 32 × 32 resampled channels. Each field will use a 64 × 64 fluence map. Each manual-planning episode will start from seven fixed coplanar fields at approximately equal angular separation. The primary synthetic comparison will not include patient-specific beam-angle changes. Earlier anatomy-only angle rules did not predict dosimetric improvement with adequate reliability. The TCIA development pilot permits only the prespecified seven-, nine-, and 12-field template sequence after unresolved hard failures. It does not search individual angles. Patient-specific beam-angle selection will remain a separate prespecified experiment after the target-structure and priority task passes. Noncoplanar actions will not be included in the primary experiment.
...
Per-protocol acceptance will require all nine institutional objectives and a 57 Gy covering-isodose-volume to PTV-volume ratio no greater than 1.10. The delivery template used for the terminal plan will be recorded separately. A separate acceptable-variation category will be used for target-OAR conflicts. The automatic manual-planning stop requires PTV V57 Gy of at least 98%. PTV V57 Gy can decrease to at least 95% but less than 98% only when an OAR is at or above its institutional limit. Such a simultaneous target and OAR variation requires physician review and is not accepted automatically. Rectum or bladder volume can exceed an institutional volume limit by no more than 5 percentage points only when standard target coverage passes. PTV D1cc at most 63 Gy is a hard limit. A plan above this limit must be replanned and cannot be accepted as a variation or sent forward as a target–OAR trade-off. Failure to resolve the hotspot with the permitted actions produces an unresolved planning-failure label. Prostate V60 Gy at least 99%, PTV minimum dose greater than 54 Gy, femoral-head limits, and conformity will not be relaxed. This quantitative policy includes expert-review rules and adapts the PERYTON 60 Gy in 20 fractions variation ranges. It is consistent with PACE guidance that permits limited PTV undercoverage at rectal overlap. The original prostate and PTV remain unchanged for evaluation. Paddick conformity index, R50, PTV D98, PTV D50, and PTV D2 will remain descriptive secondary metrics. These criteria define acceptability within the research dose surrogate. They do not establish that a plan is suitable for patient treatment.
...
5. increase normal-tissue priority;
6. replace a failed seven-field template with the fixed nine-field template, or replace a failed nine-field template with the fixed 12-field template; or
7. stop and accept the current plan, permitted only when all acceptance criteria are satisfied.

The 15-patient development calibration will use four starting profiles assigned before dose optimization: balanced reference, guarded OAR stress, hotspot stress, and conformity stress. The balanced profile sets all planner-controlled priorities to 1.0. Each stress profile sets target priority to 3.0. The OAR, hotspot, and normal-tissue priorities are 0.10, 0.04, and 0.02, respectively. These values were selected in the development set to produce near-limit initial failures without exceeding the fixed severity bounds. If the related clinical metric fails, the reviewer increases the active priority by a factor of 3.0. The maximum priority is 22.78125. Development tests of further hotspot-priority increases to 34.171875 and 68.34375 did not resolve the hotspot; the larger value produced an invalid dose. Both extensions were rejected. A safety cap of 32 manual changes prevents an endless calculation. Clinical stopping rules and action-response rules normally stop the sequence earlier. A hard target or hotspot failure can continue to the maximum priority even after two subthreshold responses. These starting states and changes are high-level planning settings. They are not selected by an optimizer search.
...
The reviewer will first correct any PTV minimum-dose, prostate-coverage, PTV absolute-coverage, or hotspot failure. If an OAR objective fails and the PTV margin overlaps that OAR, the reviewer will create the corresponding PTV-minus-OAR optimization target before it declares an anatomical conflict. The reviewer will then change the named OAR and target priorities to move the plan toward the target–OAR boundary. A geometry-only lower bound is retained as a diagnostic. It does not stop the sequence before the split target and permitted priority changes have been tried. The reviewer calculates the relative deviation from each failed clinical limit. Hard target and hotspot failures have priority over OAR and conformity changes. A priority at its ceiling is skipped so that the reviewer can act on the next failed objective. A plan that fails only conformity receives a normal-tissue-priority increase. If a hard failure remains after the seven-field actions, the reviewer starts a new plan with the prespecified nine-field template and reapplies the fixed starting objectives. If the nine-field plan retains a hard failure, the reviewer applies the equivalent 12-field template. These templates are fixed before the run. The reviewer does not search beam candidates. The 36-angle arc-like feasibility model is not part of the action vocabulary. Each plan is recalculated from the same fluence initialization after a manual change. This removes optimizer warm-start history from comparisons between manual settings.

The response to each manual action is measured before another action of the same class is selected. A target-priority action must increase PTV minimum dose by at least 0.3 Gy when the minimum-dose rule fails. It must otherwise increase prostate V60 Gy or PTV V57 Gy by at least 1 percentage point. A hotspot-priority action must reduce PTV D1cc by at least 0.3 Gy. An OAR-priority or PTV-minus-OAR action must reduce the relevant OAR volume by at least 1 percentage point or by at least 10% of the current excess, whichever is larger. A normal-tissue-priority action must reduce the 57 Gy covering-isodose ratio by at least 0.01. A beam-template action uses the response measure for the hard failure that triggered the replan: 0.3 Gy for D1cc or PTV minimum dose, or 1 percentage point for PTV V57 Gy or prostate V60 Gy. If two consecutive actions of the same class do not meet the applicable response threshold, that action class is stopped. The reviewer then changes strategy or records that physician review is required. The second repeated nonresponsive action and the complete affected episode are retained in the calibration record but are excluded from expert demonstration labels.

The procedure stops when the plan passes; it does not apply an additional change without a corresponding violation. A nonacceptable terminal plan is assigned to one of three groups: a correctable planning failure not reached within the action budget, a major target-OAR variation that requires physician review, or a technical failure. For a major variation, the record preserves a target-preserved state and an OAR-preserved state. It reports PTV V57 Gy and the worst OAR objective ratio for both states. It does not assign an automatic accept label. Thus, one retained trajectory transition represents one interpretable beam-template, target-structure, or priority change followed by automated reoptimization.
...
Primary demonstrations will be generated by sequential review of dose planes, exact 54, 57, 60, and 63 Gy isodose values, cumulative DVHs for the prostate, PTV, bladder, rectum, and separate femoral heads, and a table that shows every active evaluator value and limit. The reviewer will select one permitted high-level action after each plan. The action and a short reason will be stored before the inner optimizer is rerun. A calibration set will be reviewed jointly with a medical physicist until the action rubric is frozen. Reviewer agreement and all disagreements will be reported. The terminal disposition will state whether the plan is acceptable, is at a target–OAR boundary that requires physician review, has an unresolved hard planning failure, or has a technical failure. A 32-change cap protects against an endless sequence. These labels will not be interpreted as proof of physical infeasibility. Repeated actions without a material response will not be used as expert demonstrations.
...
The current verification suite contains 60 passing tests. The chronological development record below retains the smaller test count that was current at its earlier checkpoint.
...
The first 101-patient fixed-profile run completed in 3544.8 s and produced 91 automatic acceptances and 10 physician-review cases. Expert review of the 10 cases identified eight correctable replanning failures and two target–bladder trade-offs. The review changed the PTV-overlap minimum dose, stopping order, hard-failure handling, action cap, and priority ceiling. The run is therefore a methods audit and not an independent validation result. A new external cohort is required for the final clinical-anatomy performance estimate.

The subsequent ten-case development replan enforced PTV D1cc at most 63 Gy and PTV minimum dose greater than 54 Gy. Three seven-field plans were rejected for unresolved hotspots. Further hotspot-priority increases did not correct these plans. Fixed nine-field templates corrected Prostate-AEC-060 and Prostate-AEC-076, and a fixed 12-field template corrected Prostate-AEC-097. Their terminal PTV D1cc values were 61.99, 61.56, and 60.90 Gy. Prostate-AEC-042 also passed after replacement with the nine-field template. Prostate-AEC-073 remained below the 95% PTV V57 Gy floor with seven, nine, 12, and 36-angle arc-like plans; its best PTV V57 Gy was 94.82% with 12 fields. The arc-like condition was a fixed-angle feasibility model and not VMAT. The integrated rule replay required 1730.6 seconds on an NVIDIA GeForce RTX 4060. Three plans reached the acceptable OAR-variation class, six reached the target-OAR physician-review boundary, and one remained an unresolved PTV-coverage failure. Five terminal plans used seven fields, three used nine fields, and two used 12 fields. The maximum terminal PTV D1cc was 62.89 Gy. No hotspot failure was accepted or assigned to physician review. These results were used to verify the recorded seven-to-nine-to-12-field escalation rule. They are development findings and not independent estimates of clinical performance.
