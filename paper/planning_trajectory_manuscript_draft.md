...
Each priority action applies a discrete multiplicative increment selected before dataset generation. The provisional increment is 1.75. For example, a failed target-coverage review changes the target priority from 1.00 to 1.75. If target coverage remains below the declared tier after reoptimization, a second action changes the target priority from 1.75 to 3.0625. The procedure stops when the plan passes; it does not apply an additional increase without a remaining target violation. Each beam action changes one recorded gantry angle by the frozen increment. The inner optimizer is then rerun, and the resulting optimized state is recorded. Thus, one trajectory transition represents one interpretable change to beam geometry or a named planning priority followed by automated reoptimization.
