...
- Fifty-nine passing tests with CUDA-enabled Torch, including NumPy/PyTorch forward and adjoint parity, batched-state agreement, inactive-beam masking, clinical evaluator checks, physical-volume D1cc, trial-variation classification, split integrity, geometric objective-conflict detection, field-template escalation, and high-level action integrity.
...
The active research review now reports three separate classes: per protocol; acceptable target-coverage variation; and acceptable OAR variation. PTV D1cc at most 63 Gy is a hard limit. A plan above this limit is never accepted and is not sent forward as a clinical trade-off. The planner must replan; if the permitted actions do not reduce D1cc to 63 Gy or less, the terminal label is an unresolved planning failure. Prostate V60 Gy at least 99%, femoral-head limits, and conformity also remain fixed. Target and OAR variations cannot occur together in the automatic acceptable class. A larger target–OAR deviation is labeled major variation and requires physician review. The geometry audit now stops a case only when neither allowed variation path can be feasible.

The ten-case expert replan audit identified a hotspot that remained at 63.91 Gy after hotspot-priority changes from 1.0 to 3.0, 9.0, and 22.78125. The plan was correctly labeled as an unresolved planning failure. A further increase to 68.34375 produced a grossly invalid dose. A smaller increase to 34.171875 remained numerically valid but worsened D1cc from 63.91 Gy to 64.22 Gy. Both extensions were rejected. The priority ceiling remains 22.78125. The next development test changes the beam arrangement without relaxing the 63 Gy hard limit.

Prespecified higher-field templates corrected four of five residual seven-field hard failures. Prostate-AEC-042 passed with nine fields. Prostate-AEC-060 and Prostate-AEC-076 reached target–bladder physician-review boundaries with nine fields; their PTV D1cc values were 61.99 Gy and 61.56 Gy. Prostate-AEC-097 required 12 fields and reached a physician-review boundary with PTV D1cc 60.90 Gy, PTV minimum dose 54.52 Gy, PTV V57 Gy 95.86%, and conformity ratio 1.049. Prostate-AEC-073 remained a target-coverage planning failure. Its best result used 12 fields and had PTV V57 Gy 94.82%; a 36-angle arc-like feasibility plan reduced coverage to 94.30%. The arc-like model is not a true VMAT optimizer and is not included in the manual action set.

The revised target–OAR boundary also corrected the AEC-091 stopping path. After one PTV-minus-bladder action, PTV D1cc was 62.89 Gy, PTV minimum dose was 55.51 Gy, PTV V57 Gy was 96.24%, and the conformity ratio was 1.000. The plan was sent for physician review because the bladder objective remained above its limit. The prior unstable extra reoptimizations are not retained as demonstrations.
...
The current development configuration removes the PTV-OAR margin overlap from the full-dose optimization target and applies a 54 Gy minimum-dose objective to the overlap. The original PTV remains the evaluation structure. Automatic stopping requires PTV V57 Gy of at least 98%. An OAR-limited plan can reach a physician-review boundary with PTV V57 Gy of at least 95% but less than 98%, but PTV minimum dose must remain greater than 54 Gy. Prostate V60 Gy at least 99% and PTV D1cc at most 63 Gy are hard limits. A simultaneous target and OAR variation is not accepted automatically.

The first 101-patient run with the locked profile manifest completed in 3544.8 seconds on the local RTX 4060. It produced 91 automatic acceptances and 10 physician-review cases. The margin-only stratum had 31 of 31 automatic acceptances. The interface-overlap stratum had 60 of 70. The median corrected trajectory contained two manual changes. This run used an eight-change limit, allowed zero dose in the separated PTV-OAR overlap, and stopped two cases at a geometry-only conflict before any manual action.

Expert review of the ten nonautomatic plans found eight correctable replanning failures and two target-bladder trade-off cases. The review identified four method errors: the geometry-only conflict check occurred before the PTV-minus-OAR action; the overlap had no minimum-dose objective; the eight-change cap could stop a correctable plan; and the target and hotspot weight ceilings could stop while a hard target objective still failed. The ten decisions are stored in `data/manifests/tcia_locked101_expert_review_20260811.csv`.

The revised reviewer must try the applicable PTV-minus-OAR structure before it declares an anatomical conflict. It continues while a useful permitted action remains, with a 32-change safety cap. Hard PTV minimum-dose, prostate-coverage, absolute PTV-coverage, and hotspot failures have priority over OAR and conformity changes. PTV D1cc greater than 63 Gy and PTV minimum dose of 54 Gy or less are planning failures, not physician trade-offs. Because the ten reviewed cases changed these rules, the 101-patient run is now a methods audit and not an independent final validation result.

The integrated ten-case replan with the fixed seven-to-nine-to-12-field escalation completed in 1730.6 seconds on the local RTX 4060. Three plans reached the acceptable OAR-variation class, six reached the prespecified target-OAR physician-review boundary, and one remained a planning failure because PTV V57 Gy was 94.82%, below the 95% floor. Five terminal plans used seven fields, three used nine fields, and two used 12 fields. The three cases that had unresolved seven-field hotspots were corrected to PTV D1cc values of 61.99, 61.56, and 60.90 Gy. The maximum terminal PTV D1cc across all ten plans was 62.89 Gy. No hotspot failure was accepted or assigned to physician review. The run is stored in `outputs/tcia_expert_replan10_field_escalation_20260811_202942`. This selected development cohort tests the corrected rules and does not estimate population performance.
